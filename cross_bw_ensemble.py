#!/usr/bin/env python3
"""
cross_bw_ensemble.py
====================
Cross-Bin-Width Soft-voting Ensemble.
...
feature selection (different GA rankings per BW) AND feature values.

Usage:
    conda activate gpu-radiomics
    python cross_bw_ensemble.py
"""
...
WORKSPACE_DIR: Path = Path(__file__).parent / "workspace" / "original_only"
...
# ── Configuration ───────────────────────────────────────────────────────────
TARGET_BWS: list[int] = [10, 15, 20, 30, 50] # [5, 10, 15, 20, 25, 30, 35, 40, 45, 50][20, 35, 40]
...
if __name__ == "__main__":
    print("="*70)
    print("  Cross-BW Soft-voting Ensemble (each BW uses own splits)")
